...
resource "aws_ssoadmin_managed_policy_attachment" "developer_poweruser" {
  # Destroy attachment before account assignment (Hashicorp aws_ssoadmin_account_assignment docs).
  depends_on = [aws_ssoadmin_account_assignment.developers]
...
resource "aws_ssoadmin_permission_set" "jml_audit_readonly" {
  ...
  description      = "Read-only access to JML onboarding/offboarding/license-reclaim DynamoDB audit tables (GRC)."
  instance_arn     = local.instance_arn
  session_duration = "PT8H"
}

# Inline policy mirrors terraform/aws-grc-audit/iam_role.tf (ohmgym-grc-jml-audit-read).
...
resource "aws_ssoadmin_permission_set_inline_policy" "jml_audit_readonly" {
  depends_on = [aws_ssoadmin_account_assignment.grc]
