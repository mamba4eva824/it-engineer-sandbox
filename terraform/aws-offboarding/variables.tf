# Input variables for the ohmgym-offboarding-workflow stack.
#
...
variable "name_prefix" {
  ...
  type        = string
  default     = "ohmgym-offboarding-workflow"
}

variable "dynamodb_table_name" {
  description = "Audit-trail table that records every deactivation attempt."
  type        = string
...
variable "tags" {
  ...
  default = {
    ...
    Component = "offboarding-workflow"
    ManagedBy = "terraform"
    Owner     = "it-ops"
  }
}
...
variable "schedule_cron" {
  description = "EventBridge Scheduler cron expression in `cron(...)` form. 5pm daily by default."
  type        = string
  default     = "cron(0 17 * * ? *)"
}
