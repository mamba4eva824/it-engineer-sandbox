# Lambda function + log group for the ohmgym offboarding workflow.
#
# Source code lives at lambdas/offboarding_workflow/. Run build.sh once
...
locals {
  lambda_zip_path = "${path.module}/../../lambdas/offboarding_workflow/build/handler.zip"
}
...
resource "aws_lambda_function" "offboarding_workflow" {
  function_name = var.name_prefix
  description   = "Proactively deactivates Okta ACTIVE users whose profile.endDate matches today (PT) and posts a batch summary to #leaver-it-ops."
  ...
  environment {
    variables = {
      ...
      DYNAMODB_TABLE_NAME              = aws_dynamodb_table.offboarding_logs.name
...
resource "aws_lambda_permission" "allow_scheduler" {
  ...
  function_name = aws_lambda_function.offboarding_workflow.function_name
