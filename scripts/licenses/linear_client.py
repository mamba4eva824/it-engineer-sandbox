...
def _linear_post(api_key: str, payload: dict[str, Any], timeout: int = 15):
...
def _linear_http_error(resp: requests.Response) -> dict[str, Any]:
    if resp.status_code in (401, 403):
        return finding(
            app="linear",
            status="error",
...
def suspend_user(
    *,
    ...
    """Look up the human by email, then userSuspend. Already inactive is idempotent."""
    try:
        resp = _linear_post(api_key, {"query": QUERY})
    except (requests.Timeout, requests.ConnectionError) as exc:
        return finding(
            app="linear",
            status="error",
    ...
    if resp.status_code != 200:
        return _linear_http_error(resp)
    ...
    if not match or not match.get("id"):
        return finding(
            ...
            action_hint=ACTION_HINT,
            http_status=resp.status_code,
            error="Linear user already absent",
        )
    if match.get("active") is False:
        return finding(
            ...
            action_hint=ACTION_HINT,
            http_status=resp.status_code,
            error="Linear user already suspended",
        )

    try:
        mut = _linear_post(
            api_key,
            {"query": SUSPEND_MUTATION, "variables": {"id": match["id"]}},
        )
    except (requests.Timeout, requests.ConnectionError) as exc:
        return finding(
            app="linear",
    ...
    mut_errors = mut_body.get("errors") or []
    success = ((mut_body.get("data") or {}).get("userSuspend") or {}).get("success")
    if success:
        return finding(
            app="linear",
            status="reclaimed",
    ...
    err = mut_errors[0] if mut_errors else "Linear userSuspend did not succeed"
    return finding(
        app="linear",
        status="error",
        seat_type=SEAT_TYPE,
